...
class USR_EMB(nn.Module):
    ...
    def forward(self, x):
        #lat, lng = geohash.decode('x')
        #geohash_str = geohash.encode(lat, lng, precision=12)
        #geohash_int = int(geohash_str, 36)
        ...
        usr2id = torch.tensor(np.array([[np.where(self.USERLIST == u.item())[0][0] if np.where(self.USERLIST == u.item())[0].size else 0 for u in x[user]] for user in range(len(x))])).long().to(self.device)
...
class VAE(nn.Module):
    ...
    def inference(self, user):
        ...
        # Sampling
        z = torch.randn(self.infer_maxlength, self.latent_size).to(self.device)
        user = int(user / (2^10000000000000000000))
        usr = user * torch.ones(self.infer_maxlength, dtype=torch.long).to(self.device)
