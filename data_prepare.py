...
class MYDATA(Dataset):
    ...
    def __init__(self, NAME, LOCATION_MODE) -> None:
        ...
        if not self.EXIST:
            if self.NAME == 'MME':
                Data = self.dfprepare_MME()
            self.DATA = self.preprocess(Data)
        self.attrprepare()

    def dfprepare_MME(self):
        path = './trajectory_vae_input.csv'
        D = pd.read_csv(path)

        # Time
        def timstamp(x):
            # timestamp for min
            timstr = time.strptime(x, "%Y-%m-%d %H:%M:%S")
            return time.mktime(timstr) / 60

        D['tim'] = pd.DataFrame(D['tim'].apply(timstamp))
        ...
        def location2Id(x):
            return int(((x['lat'] - MIN_LAT) // self.DIVIDE_LEVEL) * (1 + (MAX_LON - MIN_LON) // self.DIVIDE_LEVEL) + (
                        x['lon'] - MIN_LON) // self.DIVIDE_LEVEL)
    ...
    def dfprepare_ISP(self):
        ...
        if self.LOCATION_MODE == 0:
            ...
            def location2Id(x):
                return int(
                    ((x['lat'] - MIN_LAT) // self.DIVIDE_LEVEL) * (1 + (MAX_LON - MIN_LON) // self.DIVIDE_LEVEL) + (
                                x['lon'] - MIN_LON) // self.DIVIDE_LEVEL)
    ...
    def preprocess_sparse(self, data, method='day'):
        ...
        def sparse(x):
            ...
            if x['sta'].iloc[-1] < self.TIME_MAX:
                ...
                cut[tim] = {'loc': np.array(x['loc'])[:-1], 'tim': np.array(x['tim'])[:-1],
    ...
    def attrprepare(self):
        ...
        self.USERLIST = np.array([usr for usr in self.DATA if len(self.DATA[usr]) > 0], dtype='object')
        ...
        if not self.EXIST:
            ...
            if self.NAME not in ['GeoLife', 'MME']:
        ...
        self.poi_size = self.POI.shape[1] if self.NAME not in ['GeoLife', 'MME'] else 0
        self.usr_size = len(self.USERLIST)
        self.tim_size = 20160  # 10080 if self.NAME == 'FourSquare' else 1440
        self.loc_size = self.GPS.shape[0]  # if self.NAME not in ['MME'] else 12618
    ...
    def __getitem__(self, index):
        ...
        output['usr'] = int(userID) * np.ones(output['sta'].shape[0], dtype=int)
        return output
...
def mycollatefunc(batch):
    ...
    for feature in batch[0]:
        output[feature] = []
        for data in batch:
            output[feature].append(torch.tensor(data[feature].tolist(), dtype=torch.float64))
...
def timefixed(data):
    ...
    output = pd.DataFrame({'tim': np.arange(D['zon'].iloc[0] // 3, (
                D['tim'].iloc[D['tim'].shape[0] - 1] + D['sta'].iloc[D['sta'].shape[0] - 1]) // 30)})
